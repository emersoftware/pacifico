...
export const content = {
  en: {
    ...
    howBody:
      'Pacifico imports the session files your agents already save. You can search those copies after the original files are gone.',
    flow: [
      {
    ...
    toc: ['Requirements', 'Agent setup', 'Automatic imports', 'Check installation', 'Troubleshooting', 'Stored files'],
    ...
    daemonBody:
      'On macOS, start the daemon to import new and changed sessions every 30 seconds. This background service runs while you’re logged in. The first import may take longer.',
    daemonLabel: 'Start automatic imports',
    daemonStop: 'To stop automatic imports, run',
    linux:
      'On Linux, use pacifico daemon run for a single import. Automatic service installation is currently macOS-only.',
    ...
    verifyNote:
      'After you start the daemon, look for installed: true and scheduled: true. A completed lastRun with ok: true means the import succeeded.',
    ...
    commands: [
      ['pacifico "database migration"', 'Search your session history'],
      ['pacifico context', 'Get context for the current repository'],
      ['pacifico vault status', 'Check the session archive'],
      ['pacifico daemon run', 'Import once, then exit'],
      ['pacifico --mcp', 'Run the MCP server over stdio'],
      ['pacifico --help', 'See the full command reference'],
    ],
  ...
  es: {
    ...
    howBody:
      'Pacifico importa los archivos de sesión que tus agentes ya guardan. Puedes buscar en esas copias aunque los archivos originales ya no existan.',
    flow: [
      {
    ...
    setupNote:
      'La importación automática y los hooks de inicio de sesión son opcionales. El instalador deja ambos desactivados.',
    daemonTitle: 'Importa sesiones automáticamente',
    daemonBody:
      'En macOS, activa el daemon para importar sesiones nuevas y modificadas cada 30 segundos. Este servicio funciona en segundo plano mientras tu sesión está abierta. La primera importación puede tardar más.',
    daemonLabel: 'Activar importación automática',
    daemonStop: 'Para detener la importación automática, ejecuta',
    linux:
      'En Linux, usa pacifico daemon run para una sola importación. La instalación automática del servicio está disponible solo en macOS.',
    ...
    verifyNote:
      'Después de activar el daemon, busca installed: true y scheduled: true. Un lastRun completado con ok: true indica que la importación terminó sin errores.',
    ...
    commands: [
      ['pacifico "migración de base de datos"', 'Buscar en tu historial de sesiones'],
      ['pacifico context', 'Recuperar contexto del repositorio actual'],
      ['pacifico vault status', 'Revisar las sesiones archivadas'],
      ['pacifico daemon run', 'Importar una vez y terminar'],
      ['pacifico --mcp', 'Ejecutar el servidor MCP por stdio'],
      ['pacifico --help', 'Ver la referencia completa de comandos'],
    ],
