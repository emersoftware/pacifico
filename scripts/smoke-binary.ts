...
import { SearchSessionsOutput, GetSessionMessagesOutput } from '@pacifico/agents/mcp-schemas';
...
const env = {
  ...
  SESSIONS_OLLAMA_URL: 'http://127.0.0.1:1',
};
...
try {
  ...
  const search = SearchSessionsOutput.parse(found.structuredContent);
  assert.equal(search.results.length, 1);
  const hit = search.results[0]!;
  const read = await client.callTool({ name: 'get_session_messages', arguments: { filePath: hit.filePath, limit: 5 } });
  assert.ok(!read.isError);
  assert.ok(
    JSON.stringify(GetSessionMessagesOutput.parse(read.structuredContent)).includes('durable transcript vault'),
  );
  ...
  process.stdout.write(
    'Binary smoke: install twice, foreign config preservation, MCP handshake, 12 tools, search/read, native source unchanged, durable uninstall passed.\n',
  );
} finally {
