...
To update the executable and bundled skills:

```sh
brew upgrade pacifico && pacifico install
```

Homebrew integrations and the daemon use its stable `opt` path, so removing an old version does not break them.
