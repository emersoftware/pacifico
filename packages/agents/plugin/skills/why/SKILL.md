...
2. **Read the evidence.** The result carries the resolved `commit` (subject, author
   time, files, trailers; `null` for the free-text form) and ranked `pacifico`. Trust
...
- Empty `pacifico` is a real answer: say no session correlates, rather than guessing.
