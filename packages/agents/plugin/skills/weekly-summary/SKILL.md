...
6. **Surface new memory candidates.** Run `pacifico memory mine --all --since-last --json`, then `pacifico memory pending --json`. The mine picks up only transcripts that changed since the last run, so this is cheap; `--all` is deliberate — a weekly summary spans every project, not whichever repo you happen to be sitting in.

   `--all` advances the watermark for **every** repo, not just this one. That is the point (nothing gets skipped), but it means a later `pacifico memory mine --since-last` inside any single repo reports nothing changed until that repo's transcripts move again — this step has already mined them. Say nothing about it here; it is noted so the interaction is not rediscovered as a bug.
