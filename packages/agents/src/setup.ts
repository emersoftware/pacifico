...
const PLUGIN_VERSION = '0.1.0'; // x-release-please-version
...
function findPluginSource(): string {
  const candidates = [
    ...
    join(dirname(Bun.main), '..', 'plugin'),
    join(dirname(Bun.main), '..', 'share', 'pacifico', 'plugin'),
  ];
...
function writeMarketplaceJson(): void {
  const marketplace = {
    ...
    owner: { name: 'Pacifico contributors' },
...
function sessionsCommand(): string {
  ...
  // inherited by an editor. Use the exact executable that performed setup.
...
function shouldEnableHook(opts: SetupOptions): boolean {
  ...
  process.stderr.write(
    `\n  ${C.bold}Auto-inject a context primer at session start?${C.reset}\n` +
      `  ${C.dim}Runs \`pacifico context --hook\` on every Claude Code session start.${C.reset}\n` +
      `  ${C.dim}Costs a small number of tokens each session. Reversible via \`pacifico uninstall\`.${C.reset}\n` +
...
export function runSetup(opts: SetupOptions = {}): void {
  ...
  for (const tool of detected) {
    ...
    if (tool.id === 'pi') {
      ...
      for (const r of links.filter((l) => l.status === 'refused')) {
        w(
          `  ${C.yellow}!${C.reset} Left ${C.dim}${join(piSkillsDir(), r.name)}${C.reset} alone — not a pacifico link\n`,
        );
      }
    }
  }
  ...
  if (claudeDetected && shouldEnableHook(opts)) {
    ...
    w(`  ${C.dim}  Disable any time with \`pacifico uninstall\`.${C.reset}\n`);
  }
  ...
  if (detected.some((t) => t.id === 'claude' || t.id === 'pi') && skills.length) {
    ...
    w(`\n  ${C.dim}Run \`pacifico setup\` again after upgrading to update skills.${C.reset}\n\n`);
  }
}
