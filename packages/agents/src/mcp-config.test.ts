...
describe('codex config.toml merge', () => {
  test('merges into an existing config with prior entries, preserving all of them', () => {
    ...
    expect(after).toContain('[mcp_servers.pacifico]');
  ...
  test('backs the file up before the first edit', () => {
    ...
    expect(readFileSync(codexPath + '.pacifico-bak', 'utf-8')).toBe(REAL_CONFIG);
  });
  ...
  test('updates a stale command in place without touching the user keys beside it', () => {
    writeFixture(
      codexPath,
      `${REAL_CONFIG}
[mcp_servers.pacifico]
command = "/old/path/pacifico"
args = ["--mcp"]
startup_timeout_sec = 30.0

[mcp_servers.pacifico.env]
SESSIONS_DEBUG = "1"
`,
    );
    expect(wireCodex(codexPath, CMD).status).toBe('added');
    ...
    const after = readFileSync(codexPath, 'utf-8');
    expect(after).toContain(`command = "${CMD}"`);
    expect(after).not.toContain('/old/path/pacifico');
  ...
  test('adds a missing args key to a half-written table', () => {
    writeFixture(codexPath, `[mcp_servers.pacifico]\ncommand = "${CMD}"\n`);
    expect(wireCodex(codexPath, CMD).status).toBe('added');
  ...
  test('creates the file when Codex has no config yet', () => {
    mkdirSync(join(fixtureRoot, '.codex'), { recursive: true });
    expect(wireCodex(codexPath, CMD).status).toBe('added');
    expect(readFileSync(codexPath, 'utf-8')).toBe(`[mcp_servers.pacifico]\ncommand = "${CMD}"\nargs = ["--mcp"]\n`);
  });

  test('quotes a command containing spaces and quotes', () => {
    const odd = '/Applications/My "Tools"/pacifico';
    expect(mergeCodexConfig('', odd)).toMatchObject({
      text: `[mcp_servers.pacifico]\ncommand = "/Applications/My \\"Tools\\"/pacifico"\nargs = ["--mcp"]\n`,
    });
  });
...
describe('codex config.toml refusals', () => {
  ...
  const malformed: [string, string][] = [
    ...
    ['mcp_servers as a dotted key', 'mcp_servers.pacifico = { command = "/bin/x" }\n'],
    ['mcp_servers as an inline table', 'mcp_servers = { pacifico = { command = "/bin/x" } }\n'],
    ['an unterminated table header', '[mcp_servers.pacifico\ncommand = "/bin/x"\n'],
    ['our table as an array of tables', '[[mcp_servers.pacifico]]\ncommand = "/bin/x"\n'],
  ];
  ...
  for (const [label, content] of malformed) {
    test(`refuses ${label} rather than corrupting it`, () => {
      ...
      expect(existsSync(codexPath + '.pacifico-bak')).toBe(false); // never even opened for writing
    });
  }
  ...
  test('a refusal never writes a partial table', () => {
    ...
    wireCodex(codexPath, CMD);
    expect(readFileSync(codexPath, 'utf-8')).not.toContain('pacifico');
  });
});
...
`,
    ...
    const after = readFileSync(codexPath, 'utf-8');
    expect(after).not.toContain('pacifico');
...
describe('json clients', () => {
  ...
  test('removal takes only our server', () => {
    writeFixture(cursorPath, JSON.stringify({ mcpServers: { workos: { url: 'x' }, pacifico: { command: CMD } } }));
    expect(unwireJsonClient(cursorPath).status).toBe('added');

    const after = JSON.parse(readFileSync(cursorPath, 'utf-8'));
    expect(after.mcpServers.pacifico).toBeUndefined();
    expect(after.mcpServers.workos).toEqual({ url: 'x' });
  });
});

describe('cleanDeadConfigs', () => {
  const ours = JSON.stringify({ mcpServers: { pacifico: { command: CMD, args: ['--mcp'] } } }, null, 2) + '\n';
  ...
  test('leaves a file the user made their own, even at our path', () => {
    const theirs = JSON.stringify({ mcpServers: { pacifico: { command: CMD }, other: { command: '/bin/x' } } });
  ...
  test('leaves a file with keys beyond mcpServers alone', () => {
    const theirs = JSON.stringify({ mcpServers: { pacifico: { command: CMD } }, inputs: [] });
