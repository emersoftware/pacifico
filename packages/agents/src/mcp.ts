...
import { splitFrontmatter, frontmatterDescription } from '@pacifico/core/skill-frontmatter';
...
import { why, MAX_SESSIONS } from '@pacifico/core/why/correlate';
...
function registerTools(server: McpServer): void {
  ...
  server.registerTool(
    'get_memory_recurrence',
    {
      ...
      description:
        'What recurs: freshly mined corrections classified against the memory store — the same JSON `pacifico memory report --json` emits. Three sections. `violations`: approved memories still being re-corrected after the store last saw them — a rule the user keeps breaking is a rule that is not working. `repeats`: corrections seen across 2+ sessions and 2+ dates that nobody has triaged. `fuzzy`: possible paraphrases of approved memories, below the binary\'s assert threshold — confirmation is the /memory triage skill\'s job, and a confirmed pair closes with `pacifico memory merge <memory-id> <cluster-id>`. Call this to answer "is the user still hitting a rule they already approved", or before memory triage to find merge candidates. Read-only but not free: each call re-mines the session index and writes nothing (no upsert, no watermark advance).',
      inputSchema: {
        repo: z
          .string()
          .optional()
