...
export function renderLaunchAgent(program: string[], environment: NodeJS.ProcessEnv): string {
  ...
  for (const key of ['PATH', 'SESSIONS_OLLAMA_URL', 'SESSIONS_OLLAMA_MODEL']) {
    const value = environment[key];
...
function startDaemon(): void {
  ...
  const path = plistPath();
  const program = basename(process.execPath) === 'bun' ? [process.execPath, resolve(Bun.main)] : [process.execPath];
