...
import { asJsonObject, type JsonObject } from '@pacifico/core/extract-util';
...
export function wireJsonClient(path: string, id: ClientId, command: string): WireResult {
  ...
  const current = servers.pacifico;
...
const CODEX_TABLE = 'mcp_servers.pacifico';
const BACKUP_SUFFIX = '.pacifico-bak';
...
/**
 * Add or update `[mcp_servers.pacifico]` in a Codex config, preserving the rest
...
export function mergeCodexConfig(source: string, command: string): TomlMerge {
  ...
  if (headers.some((h) => h.array && (isOurTable(h) || isOurChild(h)))) {
    return { refused: '`mcp_servers.pacifico` is an array of tables' };
  }
...
function isOurDeadFile(raw: string): boolean {
  try {
    ...
    return names.length === 1 && names[0] === 'pacifico';
  } catch {
    return false;
  }
}
