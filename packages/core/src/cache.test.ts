...
test('v12 upgrade removes vector storage without rebuilding text rows', async () => {
  writePi('vector-upgrade', [piSession({ id: 'vector-upgrade' }), piUser('u', '', 'migrationquartz')]);
  await cache.refreshIndex();
  cache.closeDb();
  ...
  previous.close();
  cache.sessionExcerpts(piPath('vector-upgrade'), 'migrationquartz', 1);
  const migrated = new Database(cache.getDbPath(), { readonly: true });
  expect(migrated.query('PRAGMA user_version').get()).toEqual({ user_version: 13 });
