...
${foot('a description of how you worked this year, not who you are · the next slide turns it into an image')}</div>`,
  ...
  cards.push({
    ...
    body: `<div class="panel center">${kicker('roll credits')}<dl class="creditlist">${creditRows
      ...
      .join(
        '',
      )}</dl><p class="foot">pacifico wrapped ${d.year} · generated ${esc(d.generatedAt.slice(0, 10))} · locally, with no telemetry<br /><a href="${SITE_URL}">${SITE_HOST}</a></p></div>`,
  });
...
<title>pacifico wrapped ${d.year}</title>
