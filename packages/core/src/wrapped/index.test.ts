...
describe('parseWrappedArgs', () => {
  ...
  test('rejects a bad --roast-with tool', () => {
    const r = Bun.spawnSync([
      'bun',
      '-e',
      "require('./packages/core/src/wrapped/index.ts').parseWrappedArgs(['--roast-with','gemini'])",
    ]);
  ...
  test('rejects a bad year in a child process', () => {
    const r = Bun.spawnSync([
      'bun',
      '-e',
      "require('./packages/core/src/wrapped/index.ts').parseWrappedArgs(['--year','20'])",
    ]);
    expect(r.exitCode).toBe(1);
  ...
  test('unknown flag dies', () => {
    const r = Bun.spawnSync([
      'bun',
      '-e',
      "require('./packages/core/src/wrapped/index.ts').parseWrappedArgs(['--nope'])",
    ]);
    expect(r.exitCode).toBe(1);
  ...
  test('--extras pointing at a missing file dies cleanly, not with a stack trace', () => {
    const r = Bun.spawnSync(
      [
        'bun',
        '-e',
        "require('./packages/core/src/wrapped/index.ts').runWrapped({tz:'UTC',stdout:true,offline:true,extras:'/nope/missing.json',noContent:true,roots:{claudeCode:'/nope',pi:'/nope',codex:'/nope'}})",
      ],
