...
const VALID_TOOLS = new Set<string>(['claude', 'codex', 'cursor', 'antigravity', 'opencode']);
...
Usage:
  pacifico context                 Primer for the current repo
  pacifico context --full          Wider per-session detail
  pacifico context --out p.md      Write to a file instead of stdout
...
export function parseContextArgs(argv: string[]): ContextArgs {
  ...
  while (i < argv.length) {
    ...
    switch (a) {
      ...
      case '--tool': {
        const v = argv[++i] ?? '';
        if (!VALID_TOOLS.has(v)) die('--tool must be claude|codex|cursor|antigravity|opencode');
      ...
      case '--here':
        ...
        // `pacifico context --here` invocation works without surprising users.
        args.here = true;
        break;
      case '--full':
        args.full = true;
