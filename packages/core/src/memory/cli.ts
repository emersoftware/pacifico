...
// interface, and `pacifico memory mine --json | <agent>` still pipes exactly as before.
...
(~/.local/share/pacifico/memory.db) that survives --clear-cache and cleanup.
...
Usage:
  pacifico memory mine             Mine the current repo
  pacifico memory mine --all       Mine every repo in the index
  pacifico memory mine --since-last  Mine only what changed since the last mine
  pacifico memory pending          Count and preview candidates awaiting triage
  pacifico memory report           What recurs: violations, untriaged repeats,
                                   fuzzy paraphrase candidates (--since YYYY-MM-DD)
  pacifico memory documented       What is already binding here (CLAUDE.md,
                                   AGENTS.md, Claude Code's own memory store)
  pacifico memory approve <id>     Keep a candidate as a durable memory
                                   (--as "<text>", --always-on,
                                    --scope group:<name>|repo:<path>)
  pacifico memory reject <id>      Dismiss a candidate; it stops being emitted
  pacifico memory snooze <id>      Hide a candidate without rejecting it
  pacifico memory merge <id> <id>...  Fold paraphrases into the first id
  pacifico memory export           Write approved memories as a portable bundle
  pacifico memory import <path>    Merge another author's bundle in as candidates
...
~/.local/share/pacifico/groups.json, e.g.
 ...
 * keeps `pacifico memory export > b.json && pacifico memory import b.json` a no-op
  ...
  if (unbound > 0) {
    ...
    process.stderr.write(
      `  ${unbound} repo-scoped ${plural} arrived unbound (the source named a project this machine cannot resolve to a path)\n` +
        ...
        `pacifico memory approve <id> --scope repo:.\n`,
    );
  }
}
