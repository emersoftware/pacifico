...
let tmp: string;
let mcp: typeof import('@pacifico/agents/mcp');
...
beforeAll(async () => {
  ...
  setMemoryEnv(tmp);
  closeDatabases();
  mcp = await import('@pacifico/agents/mcp');
...
test('driving review_memory recurrence over the protocol leaves the store byte-identical', async () => {
  const before = dumpStore();
  const client = await connect();
  ...
  for (const args of [{}, { all: true }, { cwd: '/repos/app' }]) {
    const res = await client.callTool({ name: 'review_memory', arguments: { mode: 'recurrence', ...args } });
    expect({ args, isError: Boolean(res.isError) }).toEqual({ args, isError: false });
  }
