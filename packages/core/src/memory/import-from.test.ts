...
describe('memory import --from', () => {
  ...
  test('--from all composes every fact-bearing source', async () => {
    // The test's cwd may itself contain AGENTS.md. Only fixture instructions
    // belong in this import, regardless of the repository running the suite.
    const repo = join(tmp, 'repos', 'empty');
    mkdirSync(repo, { recursive: true });
