...
// the `todayIso` discipline documented at src/memory/cli.ts:54-58 and the `nowMs`
...
/** The exact form `fingerprint` (src/memory/record.ts:34-38) produces. */
const MEMORY_ID = /^sha256:[0-9a-f]{64}$/;
 ...
 * error keep its stack (src/memory/cli.ts converts only `UsageError`, and index.ts
 ...
 *    memory with an empty key is explicitly skipped on retrieval
 *    (src/memory/retrieve.ts) rather than matching every cwd.
 *
 ...
 *    would strip the only thing that distinguishes one group from another - the memory
 ...
 *    groups.json has no such group, `groupsFor` returns nothing for it, and the memory
 ...
 */
export function toPortable(records: MemoryRecord[], exportedAt: string): MemoryBundle {
  ...
  const memories: PortableMemory[] = records
    ...
    .sort(byId);
  return { v: MEMORY_SCHEMA_VERSION, exportedAt, memories };
}

/**
 * `''` is a legal date here: src/memory/record.ts:84-85 yields an empty range when a
 ...
 * unbounded path into the store - and an approved memory is injected into every agent
 * task through `get_memory` (src/mcp.ts:54-73), so an oversized one is a context tax
 * on every future task, not a one-off.
 */
const portableMemorySchema = z.strictObject({
  v: z.literal(MEMORY_SCHEMA_VERSION),
  id: z.string().regex(MEMORY_ID),
  text: z.string().min(MIN_TEXT_LENGTH).max(MAX_TEXT_LENGTH),
  kind: z.enum(['instruction', 'information']),
  // Every member of MemoryScope['type'], restated. Widening the TypeScript union does
...
const bundleSchema = z.strictObject({
  ...
  memories: z.array(portableMemorySchema),
});
...
/**
 * Validate a parsed bundle and hand back its memory. Throws `PortableFormatError`.
 *
...
export function fromPortable<T>(bundle: T): PortableMemory[] {
  ...
  for (const memory of parsed.data.memories) {
    const derived = fingerprint(normalizeText(memory.text));
    if (derived !== memory.id) {
      throw new PortableFormatError(`memory id does not match its text (expected ${derived}, got ${memory.id})`);
    }
  }
...
export interface MergedMemory {
  ...
  kind: MemoryKind;
  scope: MemoryScope;
  /** Every author who independently produced this memory. Sorted, deduplicated. */
  authors: string[];
 ...
 * derivation rule (src/memory/mine.ts:151-156), a fact two people hit in unrelated
 ...
 *
 * The widened key is `''`, matching `deriveScope` (src/memory/mine.ts:152). Keeping
 * either author's key would be a path from someone else's machine.
 */
function widenScope(scopes: MemoryScope[]): MemoryScope {
 ...
 *
 * `''` (an undated record, src/memory/record.ts:84-85) is skipped rather than compared:
...
function mergeGroup(id: string, group: PortableMemory[]): MergedMemory {
  // Two authors can share an id while differing in `text` case: `fingerprint` folds
  // case but the stored text preserves it (src/memory/record.ts:19-27). Same for a
...
/**
 * Cluster portable memory by content-addressed identity.
 *
 ...
 * Clustering paraphrases is an LLM judgment and belongs to the `/memory` triage skill,
 * run over the merged set - a Phase 6 dependency, and only worth solving if
 * multi-author data ever actually arrives.
 */
export function merge(memories: PortableMemory[]): MergedMemory[] {
  const groups = new Map<string, PortableMemory[]>();
  for (const memory of memories) {
    const group = groups.get(memory.id);
    if (group) group.push(memory);
    else groups.set(memory.id, [memory]);
  }
...
/**
 * Turn a merged memory into a local record ready for `upsertCandidates`.
 *
 * `state` is always `candidate` and `snoozedUntil` always null (`buildRecord`'s
 * defaults). Importing is not consent: the recipient triages with `/memory` like any
 * other candidate, and nothing imported can reach an agent until they approve it
 * locally (`activeMemoryFor` filters to `approved`).
 *
 ...
 */
export function toRecord(memory: MergedMemory, local?: MemoryRecord): MemoryRecord {
  return buildRecord({
    text: memory.text,
    kind: local?.kind ?? memory.kind,
    scope: local?.scope ?? memory.scope,
    author: local?.author ?? memory.authors[0] ?? 'unknown',
    sessions: local?.evidence.sessions ?? [],
    dates: [local?.evidence.firstSeen ?? '', local?.evidence.lastSeen ?? '', memory.firstSeen, memory.lastSeen],
    distinctPhrasings: Math.max(local?.evidence.distinctPhrasings ?? 0, memory.totalPhrasings),
