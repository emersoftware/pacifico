...
// named `pacifico-<os>-<arch>` with the tokens mise recognizes.
...
const EXPECTED: ReadonlyArray<readonly [string, string]> = [
  ['bun-darwin-arm64', 'pacifico-darwin-arm64'],
  ['bun-darwin-x64', 'pacifico-darwin-x86_64'],
  ['bun-linux-x64', 'pacifico-linux-x86_64'],
  ['bun-linux-arm64', 'pacifico-linux-arm64'],
];
...
describe('release matrix', () => {
  ...
  test('linux arm64 counterpart matches x86_64 naming convention', () => {
    const linux = EXPECTED.filter(([, a]) => a.startsWith('pacifico-linux-'));
    expect(linux.map(([, a]) => a).sort()).toEqual(['pacifico-linux-arm64', 'pacifico-linux-x86_64']);
  });
});
...
describe('homebrew formula update', () => {
  ...
  test('maps each exact artifact filename to its checksum', () => {
    ...
    const keys = [...workflow.matchAll(/'(pacifico-[^']+\.tar\.gz)':/g)].map((m) => m[1]);
  ...
  test('accepts the coordinated Linux ARM64 placeholder', () => {
    ...
    expect(workflow).toContain('pacifico-linux-arm64.tar.gz');
  });
});
