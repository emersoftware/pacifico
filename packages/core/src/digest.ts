...
Usage:
  pacifico digest <file-path>      Digest a session JSONL file
  pacifico digest <session-id>     Resolve an indexed session id (newest match)
...
export function parseDigestArgs(argv: string[]): DigestArgs {
  ...
  if (!target) die('usage: pacifico digest <file-path | session-id>');
  return { target };
}
